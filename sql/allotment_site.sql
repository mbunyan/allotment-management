...
CREATE TABLE IF NOT EXISTS `allotmentdb`.`allotment_site` (
  ...
  `allotment_owner_id` INT UNSIGNED NOT NULL COMMENT 'Site Owner',
  `title` VARCHAR(150) NOT NULL COMMENT 'Site title',
  `line_1` VARCHAR(100) NOT NULL,
  `line_2` VARCHAR(100) NULL DEFAULT NULL,
  `town_city` VARCHAR(100) NOT NULL COMMENT 'Town/City',
  `postcode_zip` VARCHAR(15) NOT NULL COMMENT 'Post Code',
  `county_borough` VARCHAR(100) NOT NULL COMMENT 'County/Borough',
  `country` VARCHAR(100) NOT NULL COMMENT 'Country',
	`ward` VARCHAR(100) NOT NULL COMMENT 'Ward',
